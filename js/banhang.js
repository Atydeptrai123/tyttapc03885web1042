...
filter.onchange = function() {
    rows.forEach(row => {
        ...
        if (filter.value == 0) {
            ...
            row.style.display = Number(cell) < 1000 ? 'table-row' : 'none'
        } else if (filter.value == 2) {
            row.style.display = Number(cell) >= 1000 && Number(cell) <= 5000 ? 'table-row' : 'none'
        } else {
            row.style.display = Number(cell) > 5000 ? 'table-row' : 'none'
        }
    })
}
